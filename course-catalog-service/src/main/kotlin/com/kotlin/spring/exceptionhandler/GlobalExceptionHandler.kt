package com.kotlin.spring.exceptionhandler
...
class GlobalExceptionHandler: ResponseEntityExceptionHandler() {
    ...
    fun allExceptionHandler(ex: Exception, request: WebRequest): ResponseEntity<Any> {
        logger.error("Exception observed: ${ex.message}", ex)
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ex.message)
    }

    @ExceptionHandler(InstructorNotValidException::class)
    fun instructorNotValidExceptionHandler(ex: InstructorNotValidException, request: WebRequest): ResponseEntity<Any> {
        logger.error("Exception observed: ${ex.message}", ex)
